...
int main(){
    ...
    while(t--){
        ...
        sort(a,a+n);
        int d = 0;//So phan tu trung nhau
        for(int i = 0;i<n;i++){
            if(a[i]==a[i+1]) d++;
        }
        cout<<(a[n-1]-a[0]+1)-(n-d)<<endl;//So cac so trong khoang [min,max] - so phan tu co trong n(chieu dai mang - so phan tu trung nhau) = so phan tu phai them
    }
    return 0;
}